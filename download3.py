...
async def get_tracker_peers(tracker_url: str, info_hash: str, file_length: int, peer_id: bytes) -> List[Dict]:
...
async def download_file(file_content: bytes, output_dir: str):
    ...
    peers = await get_tracker_peers(tracker_url, info_hash, file_length, peer_id)
    if not peers:
    ...
    tasks = [
        download_worker(peer, file_content, info_hash, peer_id, work_queue, output_dir)
        for peer in peers
    ]
...
async def download_worker(peer: Dict, file_content: bytes, info_hash: str, peer_id: bytes, work_queue: WorkQueue, output_dir: str):
